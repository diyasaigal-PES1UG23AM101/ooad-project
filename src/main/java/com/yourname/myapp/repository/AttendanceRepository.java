...
public class AttendanceRepository {
    ...
    public List<AttendanceRecord> findAll() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return new ArrayList<>(
                session.createQuery("FROM AttendanceRecord", AttendanceRecord.class).list()
            );
        }
    }
    ...
    public List<AttendanceRecord> findByEmployeeId(String employeeId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return new ArrayList<>(
                session.createQuery("FROM AttendanceRecord WHERE employeeId = :eid", AttendanceRecord.class)
    ...
    public List<AttendanceRecord> findByDateRange(LocalDate from, LocalDate to) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return new ArrayList<>(
                session.createQuery("FROM AttendanceRecord WHERE attendanceDate BETWEEN :from AND :to", AttendanceRecord.class)
